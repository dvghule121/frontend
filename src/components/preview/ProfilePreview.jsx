...
const ProfilePreview = ({ data = {}, showPreviewOnly = false }) => {
  ...
  // Education exists if education array has content
  const hasEducation = hasContent(data.education);
  ...
  const handleExportPDF = async () => {
    ...
    const opt = {
      filename: `${data.full_name || "Resume"}.pdf`,
            ...
            <div className="header bg-white border-b-2 border-blue-600 px-8 py-5">
              <div className="text-center">
                <h1 className="text-3xl font-bold text-gray-900 mb-1">
                  {hasContent(data.full_name) ? data.full_name : "Your Name"}
                </h1>
                <p className="title text-lg text-blue-600 mb-3">
                  {hasContent(data.professional_title)
                ...
                <section className="section">
                  ...
                  <div className="skills flex flex-wrap gap-2">
                    {formatSkills(data.skills).map((skill, index) => (
                      <span
                        key={index}
                        className="skill-tag bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-medium"
                      >
                        {skill}
                      </span>
                    ))}
                      ...
                      <div key={index} className="experience-item">
                        <div className="flex justify-between items-start mb-1">
                          <div>
                            ...
                            <p className="company text-sm text-blue-600 font-medium">
                              {exp.company}
                            </p>
                          </div>
                          <p className="duration text-xs text-gray-600 text-right">
                            {exp.duration}
                          </p>
                        </div>
                        {(() => {
                          const toArray = (val) =>
                            Array.isArray(val)
                              ...
                              ? val.split("\n")
                              : [];
                          const items = toArray(exp.description)
                            .map((it) => it.trim())
                    ...
                    {data.education.map((edu, index) => (
                      <div key={index} className="education-item">
                        <div className="flex justify-between items-start mb-1">
                          ...
                          <p className="duration text-xs text-gray-600 text-right">
                            {edu.education_duration}
                          </p>
                        </div>
                        {hasContent(edu.education_location) && (
                          <p className="location text-xs text-gray-600 mb-1">
                        ...
                        {(() => {
                          const toArray = (val) =>
                            Array.isArray(val)
                              ...
                              ? val.split("\n")
                              : [];
                          const items = toArray(edu.description)
                            .map((it) => it.trim())
                            .filter((it) => hasContent(it));
                          return items.length ? (
                            <ul className="description list-disc pl-5 text-xs text-gray-700 space-y-0.5">
                    ...
                    {data.projects.map((project, index) => (
                      <div key={index} className="project-item">
                        ...
                        {(() => {
                          const toArray = (val) =>
                            Array.isArray(val)
                              ...
                              ? val.split("\n")
                              : [];
                          const items = toArray(project.description)
                            .map((it) => it.trim())
                            .filter((it) => hasContent(it));
                          return items.length ? (
                            <ul className="description list-disc pl-5 text-xs text-gray-700 space-y-0.5">
                        ...
                        {project.technologies && (
                          <p className="technologies text-xs text-blue-600 mt-1">
                ...
                !hasContent(data.projects) && (
                  <div className="empty-state text-center py-16">
                    ...
                    <p className="text-gray-500">
                      Start filling out the resume form to see your preview
                      here.
                    </p>
                  </div>
                )}
